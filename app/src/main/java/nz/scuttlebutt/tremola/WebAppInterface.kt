...
class WebAppInterface(private val act: Activity, val tremolaState: TremolaState, private val webView: WebView) {
    ...
    fun onFrontendRequest(s: String) {
        ...
        when (args[0]) {
            ...
            "onBackPressed" -> { // When 'back' is pressed, will close app
                ...
                if(args.size == 2){
                    val rawStr = tremolaState.msgTypes.mkGamePost(
                        "/ed25519-start: "+"onBackPressed() " +act.game?.peer_addr().toString(),
                        listOf(act.game?.peer_addr().toString())
                    )
                    val evnt = tremolaState.msgTypes.jsonToLogEntry(
                        rawStr,
                        rawStr.encodeToByteArray()
    ...
    private fun checkGameEntries(entry: LogEntry) {
        if(entry.pri?.removePrefix("{\"type\":\"game\",\"text\":\"\\")?.startsWith("/ed25519-start: onBackPressed()")  == true){
            val args = entry.pri?.split(" ")
            val addr = args?.get(2)?.split("\"")?.get(0)?.let { removeRedundantBackslashes(it) } //addr : receiver
            if(me == addr){
                eval("peerOnBackPressed()")
            }
        }
        else if(entry.pri?.removePrefix("{\"type\":\"game\",\"text\":\"\\")?.startsWith("/ged25519-prot: ") == true){
            val args = entry.pri?.split(" ")
        ...
        else if(entry.pri?.removePrefix("{\"type\":\"game\",\"text\":\"\\")?.startsWith("/ed25519-start: ")  == true){
